...
![StandaloneAdvanced](../images/04-CreateDeployment/Replica.png)
...
| **Replica Set Settings** | Displays an table of each process associated with the replica set. You can configure the MongoDB server version, data directory of each process. |
| **Process Name**         | Hostname and port of a `mongod` process. This hostname can be a hostname, or an IPv4 address. Whaleal initially groups each process under the replica set name. Click the replica name to display all `mongod` processes in the replica set.Whaleal applies any settings configured for the replica set to all of its associated processes. |
